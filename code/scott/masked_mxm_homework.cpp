...
namespace views
{
    ...
    public:
        ...
        ScalarType getValueAt(IndexType row, IndexType col) const
        {
            auto value = m_matrix.getValueAt(row, col);
...
namespace algorithms
{
                        ...
                        Matrix<IndexType> &levels)
    {
        ...
        graph.getShape(grows, gcols);
        wavefront.getShape(wrows, wcols);
...
int main(int, char**)
{
    // syntatic sugar (change everything to double here)
    ...
    buildMatrix(graph, i, j, v);
    printMatrix(std::cout, graph, "Test Graph:");
