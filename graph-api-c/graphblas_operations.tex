...
%{\sf extract} (column) & $\matrix{w}\langle\vector{m},z\rangle$ & $=$ & $\matrix{w}$ & $\odotsp$ & $\matrix{A}(\array{i}, j)$ \\
...
\begin{figure}
    \hrule
    \begin{center}
        \includegraphics[width=5.5in]{mxm_operation_flowchart_1_3b.pdf}
    \end{center}
    ...
    indicate where casting may occur between different domains. \scott{STRUCTURE\_ONLY changes.}}
...
{\tt GrB\_STRUCTURE\_ONLY} descriptor is not set, it is 
applied to the result from the operation wherever the mask evaluates to true.  If
the {\tt GrB\_STRUCTURE\_ONLY} descriptor is set, the mask is applied to the result
...
\begin{itemize}
\item $\vector{m} = \langle N, \{ \mathbf{ind}(\vector{v}) \} \rangle$, if 
{\tt GrB\_STRUCTURE\_ONLY} is specified,
\item $\vector{m} = \langle N, \{ i : \mbox{\tt (bool)}v_i = \true \} \rangle$, 
where $\mbox{\tt (bool)}v_i$ denotes
casting the value $v_i$ to a Boolean value (\true\ or \false), if 
{\tt GrB\_STRUCTURE\_ONLY} is not specified.
\end{itemize}
...
\begin{itemize}
\item $\matrix{M} = \langle M,N, \{ \mathbf{ind}(\matrix{A}) \} \rangle$, if 
{\tt GrB\_STRUCTURE\_ONLY} is specified,
\item $\matrix{M} = \langle M,N, \{ (i,j) : \mbox{\tt (bool)}A_{ij} = \true \} \rangle$
, where $\mbox{\tt (bool)}A_{ij}$ denotes
casting the value $A_{ij}$ to a Boolean value (\true\ or \false), if 
{\tt GrB\_STRUCTURE\_ONLY} is not specified.
\end{itemize}
\scott{STRUCTURE\_ONLY changes need to rework the itemized lists into single equations with two conditions.}
