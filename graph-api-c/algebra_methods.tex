...
\begin{itemize}[leftmargin=1.1in]
    ...
    Table~\ref{Tab:PredefinedTypes}, or a user-defined GraphBLAS type.
    \item[{\sf binary\_op}] ({\sf IN}) An existing GraphBLAS binary operator with input types that are \emph{domain compatible} with {\sf d1}, and returns a value of type {\sf d1}.
...
\item[{\sf GrB\_DOMAIN\_MISMATCH}]  the types in the operator signature are not   
                                    compatible with the {\sf GrB\_Type} parameter specified.
\end{itemize}
...
\begin{itemize}[leftmargin=1.1in]
    ...
    specifies the semiring's multiplication operator. In addition, {\sf mul\_op}'s output domain {\sf d3} must be \emph{domain compatible} with the {\sf add\_op}'s input domain {\sf d1}.
\end{itemize}
