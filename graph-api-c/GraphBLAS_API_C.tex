...
objects in GraphBLAS: algebras, vectors, matrices and descriptors. We then
...
\caption{Predefined types and corresponding domains for GraphBLAS in C. 
	...
	how a user extends the set of domains supported.}
		...
		\scott{We should add a placeholder section that will include the
		necessary API to add new domains. And the example would be nice.}}
...
\scott{Rather than a sample we need to specify the complete set of predefined operations. 
Notably missing are min and max. 
What C construct are the symbols in the second column? This specification should say.}}
...
\jose{This has been changed to algebraic structure/algebra.}
\scott{OK TO REMOVE}

A GraphBLAS \emph{algebraic structure} (or \emph{algebra} for short)
$S=\langle D_1,D_2,D_3,\oplus,\otimes,0 [,1] \rangle$ is defined by
three domains $D_1$, $D_2$ and $D_3$, an additive operation $\oplus :
D_3 \times D_3 \rightarrow D_3$, with corresponding identity $0 \in D_3$,
and a multiplicative operation $\otimes : D_1 \times D_2 \rightarrow
D_3$, with optional corresponding identity $1 \in D_1 \cap D_2$.
If $1$ is specified, then $D_1 \subseteq D_3$ and $D_2 \subseteq D_3$
must be satisfied. For a given GraphBLAS algebra $S=\langle D_1, D_2,
D_3,\oplus,\otimes,0,1 \rangle$ we define $\bold{D}_1(S) = D_1$,
...
\otimes 0 = 0, \forall x \in D_3$), a GraphBLAS algebra reduces to the
...
semiring as well} \jose{this has been fixed}. \scott{OK TO REMOVE}

\ajy{$D_i$, etc is overloaded or used recursively} \jose{this has been
fixed with explicitly different fonts} \scott{OK TO REMOVE}
...
\jose{Interesting, but we have to call it something else. Monoid only
has one set.}

\scott{Generalized Monoid, Relaxed Monoid, Binary Operator w/ Identity}

\subsection{Vectors}

\scott{Is this strictly sparse vector?}
...
Multiplies a vector by a matrix within an algebra. The result is a vector.

\paragraph{C99 Syntax}

\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_vxm(GrB_Vector *u, const GrB_Algebra s, const GrB_Vector v, 
...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Algebra used in the vector-matrix
	multiply.
...
operations of algebra {\sf s}, respectively.  If $\vector{i}(\vector{v})
...
\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_ewisemul(GrB_Vector* w, const GrB_Algebra s, const GrB_Vector u,
...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Algebra used in the vector-wise multiplication.
...
\subsection{Element-wise addition ({\sf ewiseadd})}
...
\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_ewiseadd(GrB_Vector* w, const GrB_Algebra s, const GrB_Vector u,
         const GrB_Vector v[, const GrB_Vector m[, const GrB_Descriptor d]])
\end{verbatim}
...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Algebra used in the vector-wise addition.
...
\]
where $\oplus$ is the additive operation of algebra {\sf s}.
...
\subsection{Matrix-vector multiply ({\sf mxv})}

Multiplies a matrix by a vector within a algebra. The result is a vector.

\paragraph{C99 Syntax}

\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_mxv(GrB_Vector *u, const GrB_Algebra s, const GrB_Matrix A, 
                 const GrB_vector v[, const GrB_Vector m[, const GrB_Descriptor d]])
\end{verbatim}
...
\subsection{Create new algebra ({\sf Algebra\_new})}

Creates a new algebra with specified domain, operations and identities.

\paragraph{C99 Syntax}

\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_Algebra_new(GrB_Algebra *s,GrB_type t1, GrB_type t2, GrB_type t3,
...
\begin{itemize}
	\item[{\sf t1}] The type defining the first domain of the algebra being created. Should be one of the predefined
	GraphBLAS types in Table~\ref{Tab:PredefinedTypes}, or a user created type.
	\item[{\sf t2}] The type defining the second domain of the algebra being created. Should be one of the predefined
	GraphBLAS types in Table~\ref{Tab:PredefinedTypes}, or a user created type.
	\item[{\sf t3}] The type defining the third domain of the algebra being created. Should be one of the predefined
	GraphBLAS types in Table~\ref{Tab:PredefinedTypes}, or a user created type.
	\item[{\sf a}] The additive operation of the algebra.
	\item[{\sf m}] The multiplicative operation of the algebra.
	\item[{\sf z}] The additive identity of the algebra.
	\item[{\sf o}] The multiplicative identity of the algebra.
\end{itemize}

\paragraph{Output Parameter}

\begin{itemize}
	\item[{\sf s}] Identifier of the newly created algebra.
\end{itemize}
...
Creates a new algebra $S = \langle D({\sf t}), {\sf a}, {\sf m}, {\sf z}, {\sf o} \rangle$ and
...
\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_reduce(scalar *t, const GrB_Algebra s, const GrB_Vector v)
\end{verbatim}
...
\begin{itemize}
	...
	\item[{\sf s}] Algebra defining the reduction.
\end{itemize}
...
\jose{This looks more like a \emph{function} than a \emph{monoid}.}
A GraphBLAS \emph{monoid} $M = \langle D_1,D_2,D_3,\oplus \rangle$ is
defined by three domains $D_1$, $D_2$, $D_3$ and an operation $\oplus:
D_1 \times D_2 \rightarrow D_3$.  For a given GraphBLAS monoid $M=\langle
D_1, D_2, D_3,\oplus \rangle$ we define $\bold{D}_1(M) =
