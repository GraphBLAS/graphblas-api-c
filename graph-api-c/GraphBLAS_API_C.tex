...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Space used in the vector-matrix
	multiply.
...
\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_ewisemul(GrB_Vector* w, const GrB_Space s, const GrB_Vector u,
...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Space used in the vector-wise multiplication.
...
{\sf GrB\_PANIC}	& unknown internal error \\
{\sf GrB\_OUTOFMEM}	& not enough memory available for operation \\
{\sf GrB\_MISMATCH}	& mismatch among vectors and/or space
\end{tabular}
...
where $\otimes$ is the multiplicative operation of space {\sf s}.
...
\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_ewiseadd(GrB_Vector* w, const GrB_Space s, const GrB_Vector u,
         const GrB_Vector v[, const GrB_Vector m[, const GrB_Descriptor d]])
\end{verbatim}
...
\begin{itemize}
	\item[{\sf s}] ({\sf ARG0}) Space used in the vector-wise addition.
...
\]
where $\oplus$ is the additive operation of space {\sf s}.
...
Multiplies a matrix by a vector within a space. The result is a vector.

\paragraph{C99 Syntax}

\begin{verbatim}
#include "GraphBLAS.h"
GrB_info GrB_mxv(GrB_Vector *u, const GrB_Space s, const GrB_Matrix A, 
                 const GrB_vector v[, const GrB_Vector m[, const GrB_Descriptor d]])
\end{verbatim}
