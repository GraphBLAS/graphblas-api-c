...
The argument scalar, vectors, index operator and the accumulation 
...
\begin{enumerate}
    ...
    \item $\bold{D}({\sf w})$ must be compatible with $\bDout({\sf op})$ of the index operator.

    \item If {\sf accum} is not {\sf GrB\_NULL}, then $\bold{D}({\sf w})$ must be
    compatible with $\bDin1({\sf accum})$ and $\bDout({\sf accum})$ of the accumulation operator and 
    $\bDout({\sf op})$ of the index operator must be compatible with $\bDin2({\sf accum})$ of the accumulation operator.

    \item $\bold{D}({\sf u})$ must be compatible with $\bDin1({\sf op})$ of the index operator.
    
    \item $\bold{D}({\sf val})$ must be compatible with $\bDin2({\sf op})$ of the index operator.
\end{enumerate}
...
\begin{itemize}
    \item $\vector{\widetilde{t}}$: The vector holding the result from applying the index operator to the input vector
    $\vector{\widetilde{u}}$.
...
The argument scalar, matrices, index operator and the accumulation 
operator (if provided) are tested for domain compatibility as follows:
\begin{enumerate}
    \item If {\sf Mask} is not {\sf GrB\_NULL}, and ${\sf desc[GrB\_MASK].GrB\_STRUCTURE}$
    is not set, then $\bold{D}({\sf Mask})$ must be from one of the pre-defined types of 
    ...
    compatible with $\bDout({\sf op})$ of the index operator. \scott{SHOULD THIS BE QUALIFIED: if {\sf accum} is {\sf GrB\_NULL}? Here and globally?}

    \item If {\sf accum} is not {\sf GrB\_NULL}, then $\bold{D}({\sf C})$ must be
    compatible with $\bDin1({\sf accum})$ and $\bDout({\sf accum})$ of the accumulation operator and 
    $\bDout({\sf op})$ of the index operator must be compatible with $\bDin2({\sf accum})$ of the accumulation operator.

    \item $\bold{D}({\sf A})$ must be compatible with $\bDin1({\sf op})$ of the index operator.
...
\begin{itemize}
    \item $\matrix{\widetilde{T}}$: The matrix holding the result from applying the index operator to the input matrix
    $\matrix{\widetilde{A}}$.
...
Apply a select operator to the elements of a vector.
...
This variant of {\sf GrB\_select} computes the result of applying a index operator
...
\begin{itemize}[leftmargin=2.1in]
\item[~] ${\sf w} = {\sf u}\langle f_i({\sf u}, \bold{ind}({\sf u}), {\sf val})\rangle$,
\end{itemize}
or if an optional binary accumulation operator ($\odot$) is provided:
\begin{itemize}[leftmargin=2.1in]
\item[~] ${\sf w} = {\sf w} \odot {\sf u}\langle f_i({\sf u}, \bold{ind}({\sf u}), {\sf val})\rangle$.  
\end{itemize}
...
\begin{enumerate}
	...
	\item $\bDout({\sf op})$ of the index operator must be from one of the pre-defined types of 
