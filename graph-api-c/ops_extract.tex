...
\begin{itemize}[leftmargin=2.1in]
    ...
    \item[{\sf GrB\_NULL\_POINTER}]  {\sf w} pointer is {\sf NULL}. \scott{We still
...
From the argument vectors, the internal vectors, mask, and index array used in 
...
generated.  \scott{Check this wording and make sure we say the right thing 
relative to execution models and error models.}
...
ends and the index out-of-bounds error listed above is generated.  \scott{Check 
this wording and make sure we say the right thing relative to execution models 
and error models.}
...
indices for matrix $\matrix{A}$ ($0 \leq {\sf row\_indices}[i] < \bold{m}(\matrix{A})
\forall i = 0,\ldots,{\sf nrows}-1$ and
$0 \leq {\sf col\_indices}[j] < \bold{n}(\matrix{A}) \forall j = 0,\ldots,{\sf ncols}-1$).  Otherwise, the method returns {\sf
GrB\_INDEX\_OUT\_OF\_BOUNDS}.

A new matrix $\matrix{C} = \langle \bold{D}({\sf C}),{\sf nrows},{\sf ncols},
\bold{L}(\vector{C}) = \{ (i,j,C_{ij}), i = 0,\ldots,{\sf nrows}-1, j = 0, \ldots,{\sf ncols}-1 : {\sf row\_indices}[i]
\in \bold{i}(\matrix{A}) \wedge {\sf col\_indices}[j] \in \bold{j}(\matrix{A}) \wedge \vector{Mask}[i,j] = \true \} \rangle$ is created. 
...
The value $C_{ij}$ is set to $\matrix{A}[{\sf row\_indices}[i],{\sf col\_indices}[j]]$. If
...
\begin{itemize}[leftmargin=2.1in]
    ...
    \item[{\sf GrB\_NULL\_POINTER}]    {\sf w} pointer is NULL.  \scott{We still
    do not agree if NULL {\sf row\_indices} pointer should be specified to 
...
A consistency check is performed to verify that all indices in array
{\sf row\_indices} and index {\sf col\_index} are valid. That is, they must fall within the range of allowed
indices for matrix $\matrix{A}$ ($0 \leq {\sf row\_indices}[i] < \bold{n}(\matrix{A})
\forall i = 0,\ldots,{\sf nrows}-1$ and
$0 \leq {\sf col\_index} < \bold{n}(\matrix{A})$).  Otherwise, the method returns {\sf
GrB\_INDEX\_OUT\_OF\_BOUNDS}.

A new vector $\vector{w} = \langle \bold{D}({\sf w}),{\sf nrows},
\bold{L}(\vector{w}) = \{ (i,w_{i}), i = 0,\ldots,{\sf nrows}-1 : {\sf row\_indices}[i]
\in \bold{i}(\matrix{A}) \wedge {\sf col\_index} \in \bold{j}(\matrix{A}) \wedge \vector{mask}[i] = \true \} \rangle$
is created.  The value $w_{i}$ is set to $\matrix{A}[{\sf row\_indices}[i],{\sf col\_index}]$. If
...
\scott{I argue that this should be moved to Vector methods, something like
Vector\_getElement or Vector\_getValue: (1) there are no valid descriptor flags,
(2) there is no masking, and (3) accumulation with the scalar not really that
necessary.  Is OUTOFMEMORY error possible (perhaps only in non-blocking)? }
...
The input vector used in this {\sf GrB\_extract} operation is defined as:
\[
...
\scott{I argue that this should be moved to Matrix methods, something like
Matrix\_getElement or Matrix\_getValue: (1) there are no valid descriptor flags,
(2) there is no masking, and (3) accumulation with the scalar not really that
necessary.  Is OUTOFMEMORY error possible (perhaps only in non-blocking)? }
...
The input matrix used in this {\sf GrB\_extract} operation is defined as:
\[
...
If $({\sf row\_index},{\sf col\_index}, A_{{\sf row\_index},{\sf col\_index}}) \notin \bold{L}(\matrix{A})$, 
the method returns {\sf GrB\_NO\_VALUE}.  Otherwise, if 
${\sf accum} = {\sf GrB\_NULL}$, then ${\sf val} \leftarrow \matrix{A}[{\sf row\_index},{\sf col\_index}]$.
If an accumulation function is provided, then 
${\sf val} \leftarrow {\sf accum}({\sf val},\matrix{A}[{\sf row\_index},{\sf col\_index}])$.
