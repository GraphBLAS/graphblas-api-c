...
\begin{itemize}[leftmargin=1in]
    ...
    location of elements of {\sf w} that are assigned.  If all elements of
    {\sf w} are to be assigned in order, {\sf GrB\_ALL} can be specified.
...
The value of result vector {\sf w} is undefined from this point forward in
the sequence.
\jose{Should we allow other methods in the sequence, downstream from this one but before the {\sf GrB\_Wait()},
to return the out-of-bounds error?}
\jose{I think we need an {\sf GrB\_INVALID\_OBJECT} or expand the
meaning of {\sf GrB\_NOOBJECT} for any downstream methods that use {\sf w} as input.}
...
\item[{\sf GrB\_NOOBJECT}]          one or more of the GraphBLAS objects has
                                    not been initialized by a call to {\sf new}.
\item[{\sf GrB\_NULL\_POINTER}]    {\sf C} or {\sf rowIDs} pointer is {\sf NULL}.
...
\item[{\sf GrB\_NOOBJECT}]          one or more of the GraphBLAS objects has
                                    not been initialized by a call to {\sf new}.
\item[{\sf GrB\_NULL\_POINTER}]    {\sf C} or {\sf colIDs} pointer is {\sf NULL}.
\item[{\sf GrB\_INDEX\_OUTOFBOUNDS}]  The indices specify a position that outside the dimensions of {\sf C}.
\item[{\sf GrB\_DIMENSION\_MISMATCH}] 
