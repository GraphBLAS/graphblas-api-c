...
\begin{itemize}[leftmargin=1.1in]
    ...
    passed, the following defines the binary operator, $F_b=\langle \bDout({\sf op}),\bDin1({\sf op}),\bDin2({\sf op}),\otimes\rangle$, used:
    \begin{itemize}[leftmargin=1.1in]
...
\begin{itemize}
    ...
    \item If ${\sf accum}$ is a binary operator, then vector $\vector{\widetilde{z}}$ is defined as 
        \[ \langle \bDout({\sf accum}), \bold{size}(\vector{\widetilde{w}}), \bold{L}(\vector{\widetilde{z}})
...
\begin{itemize}[leftmargin=1.1in]
    ...
    %: ${\sf accum} = \langle \bDin1({\sf accum}),
    %\bDin2({\sf accum}), \bDout({\sf accum}),\odot \rangle$. 
    If assignment rather than accumulation is
    desired, {\sf GrB\_NULL} should be specified.
...
\begin{enumerate}
	...
	\item $\bold{D}({\sf A})$ must be compatible with $\bDin1({\sf op})$ of {\sf op}.

	\item $\bold{D}({\sf B})$ must be compatible with $\bDin2({\sf op})$ of {\sf op}.
...
\begin{itemize}
    ...
    \item If ${\sf accum}$ is a binary operator, then matrix $\matrix{\widetilde{Z}}$ is defined as 
        \[ \langle \bDout({\sf accum}), \bold{nrows}(\matrix{\widetilde{C}}), \bold{ncols}(\matrix{\widetilde{C}}), \bold{L}(\matrix{\widetilde{Z}})
