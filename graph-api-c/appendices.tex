...
\begin{itemize}
\item Added {\sf GrB\_kronecker} operation.
\item Added variants of the {\sf GrB\_apply} operation that take a binary function and a scalar.
\item Added {\sf GrB\_STRUCTURE\_ONLY} descriptor flag for masks.
\item Added {\sf GrB\_LXNOR} boolean binary operator.
\item Added a set of predefined monoids and semirings.
\item Added parent-BFS example.
\item Fixed bug in the non-batch betweenness centrality algorithm in 
Appendix~\ref{App:BCnobatch} where source nodes were incorrectly assigned path counts.
\item Clarified that multiplication does not have to distribute over addition in a GraphBLAS semiring.
\item Fixed miscellaneous typographical errors (such as $\otimes.\oplus$).
\item \scott{TODO: ADD ALL CLOSED ISSUES}
\end{itemize}
