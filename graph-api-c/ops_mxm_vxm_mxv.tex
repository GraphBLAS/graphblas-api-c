...
\begin{itemize}[leftmargin=1.1in]
    ...
    \item[{\sf Mask}] ({\sf MASK}) Output mask matrix of dimensions $m \times k$. 
    ...
    \item[{\sf A}]    ({\sf ARG0}) The left-hand matrix (of dimensions $m \times n$)  to be multiplied.  
    \item[{\sf B}]    ({\sf ARG1}) The right-hand matrix (of dimensions $n \times k$) to be multiplied. 
...
A consistency check is performed to verify that inner dimensions of $\matrix{A}$
and $\matrix{B}$ agree.  If a consistency check fails, the operation is
