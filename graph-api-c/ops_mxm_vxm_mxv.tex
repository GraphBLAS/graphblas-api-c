...
If {\sf desc[GrB\_OUTP].GrB\_REPLACE} is set, then any values in $\matrix{C}$ on input to {\sf GrB\_mxm()} are deleted and the new
output matrix $\matrix{C}$ is,
\[ \bold{L}({\sf C}) = \{(i,j,Z_{ij}) : (i,j) \in (\bold{ind}(\matrix{\widetilde{Z}}) \cap \bold{ind}(\matrix{\widetilde{M}})) \}. \]
If {\sf desc[GrB\_OUTP].GrB\_REPLACE} is not set, the elements of $\matrix{\widetilde{Z}}$ indicated by the mask
...
Vectors $\vector{u}, \vector{m}$ and matrix $\matrix{A}$ are computed from
...
If either $\vector{u}, \vector{m}$ or $\matrix{A}$ cannot be computed
...
\bold{n}(\matrix{A}), \bold{L}(\vector{w}) = \{(i,w_i) : \vector{m}(i)
...
then $\vector{m}$ is a Boolean vector of size $\bold{m}(\vector{A})$
and with all elements set to {\sf true}.
...
A new vector $\vector{w} = \langle \bold{D}_3({\sf op}),
\bold{m}(\matrix{A}), \bold{L}(\vector{w}) = \{(i,w_i) : \vector{m}(i)
= {\sf true} \} \rangle$ is created.  The value of each of its elements
is computed by $w_i = \bigoplus_{j \in \vector{i}(\vector{u}) \cap
