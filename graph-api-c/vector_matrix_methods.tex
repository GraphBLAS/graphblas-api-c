...
%\item[{\sf GrB\_INVALID\_VALUE}]    {\sf w} object is already initialized.
\end{itemize}
...
This variant of {\sf GrB\_Vector\_extractElement} computes the result of 
extracting a single value from a GraphBLAS vector as follows: 
\[
    {\sf val} = {\sf u}({\sf index})
\]
where input vector used is defined as:
\[
...
This variant of {\sf GrB\_Matrix\_extractElement} computes the result of 
extracting a single value from a GraphBLAS matrix as follows: 
\[
    {\sf val} = {\sf A}({\sf row\_index}, {\sf col\_index})
\]
where the input matrix used is defined as:
\[
