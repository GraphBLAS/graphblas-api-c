...
when: (1) two or more threads access shared objects, (2) those access operations include at least one modify operation,
and (3) those operations are not ordered through synchronization operations.   The GraphBLAS C API does not provide synchronization
operations to define ordered accesses to GraphBLAS objects, hence the only way to assure race free
sequences in the GraphBLAS is to assure that whenever two or more sequences might concurrently access GraphBLAS objects 
those sequences do not modify any objects shared between the sequences.
