...
The functions from the API appear in a program.  This program is written with a programming language
...
\tim{
\glossItem{complete}  A GraphBLAS object is complete after a particular GraphBLAS method call 
when any GraphBLAS method that \emph{happens-after} said method can safely use the object as 
an {\sf IN} or {\sf INOUT} argument, even if the method calls are in different threads.  This concept is used
...
\tim{
\glossItem{materialize} A GraphBLAS object is materialized at a point in a program when it is (1) complete, (2) the computations 
defined by the sequence that define it at that point have finished (either fully or stopped at an error) and will not consume any 
...
describe  \emph{blocking} and \emph{non-blocking mode} execution modes.
...
for a single-threaded and a multi-threaded execution.   It is the application programmer's 
...
synchronized, but it expects the host programming envronment to provide
those services. 
...
must be the same when executed with a single thread or with multiple threads.
...
by the memory model of the host programming envionment.   Furthermore, a sequence
...
object is complete on return from the function {\sf GrB_complete(obj)}.
...
the method {\sf GrB_complete(obj)} is called followed by a
...
relations between method-calls on the two threads and the {\sr GrB_complete(obj)}
defines the order of memory operations on the GraphBLAS object, {\sr obj}, so the object
...
\begin{enumerate}
	...
	{\sr GrB_complete(obj)} occurs on the thread where the object was defined
