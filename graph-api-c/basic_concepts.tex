...
The multiplicative inverse ({\sf GrB\_MINV\_$T$}) function is
only defined for floating-point types ($T = $ {\sf FP32} or {\sf FP64}).
Furthermore, the division ({\sf GrB\_DIV\_$T$}) function is
only defined for integral types () if $y \neq 0$.
...
be any suffix from Table~\ref{Tab:PredefinedDomains}, and $I$ can be any integer 
suffix from Table~\ref{Tab:PredefinedDomains}.}
...
{\sf GrB\_UnaryOp}    & {\sf GrB\_BNOT}          & $I \rightarrow I$  & $f(x) =~\mbox{\~{}} x$, &bitwise complement \\
...
{\sf GrB\_BinaryOp}   & {\sf GrB\_BOR}        & $I \times I \rightarrow I$ & $f(x,y) = x ~|~ y$, & bitwise OR \\
{\sf GrB\_BinaryOp}   & {\sf GrB\_BAND}       & $I \times I \rightarrow I$ & $f(x,y) = x ~\&~ y$, & bitwise AND \\
{\sf GrB\_BinaryOp}   & {\sf GrB\_BXOR}       & $I \times I \rightarrow I$ & $f(x,y) = x ~\mbox{\^{}}~ y$, & bitwise XOR \\
{\sf GrB\_BinaryOp}   & {\sf GrB\_BXNOR}      & $I \times I \rightarrow I$ & $f(x,y) = \overline{x ~\mbox{\^{}}~ y}$, & bitwise XNOR \\
...
{\sf GrB\_BinaryOp}   & {\sf GrB\_ANY\_$T$}   & $T \times T \rightarrow T$  & $f(x,y) = $ either $x$ or $y$, & any\tnote{1} \scott{MORE INFO NEEDED FOR ANY} \\
...
\hrule
\begin{tablenotes}
	\item[1] Any means implementation is free to return any element.
\end{tablenotes}
\end{threeparttable}
\end{table}
