...
At any point during the execution of a sequence, as long as there
are no execution errors, the state of all GraphBLAS objects are
uniquely and precisely defined. In particular, as soon as a GraphBLAS
...
output object have been produced and commited to architected memory.

The opaqueness of GraphBLAS ojects allows execution to proceed
...
returning.  We note that additional internal processing, concerning only
the opaque objects, can still be deferred in those methods.
...
method are fully produced and stored in architected memory (\ie, they are
...
method, executing in blocking mode, returns with a {\sf GrB\_OUTOFMEM} or
a {\sf GrB\_INDEX\_OUTOFBOUNDS} error, it is guaranteed that no argument
used as input-only has been modified. Output arguments may be left in
an invalid state and their use downstream the program flow may cause
...
of {\sf GrB\_SUCESS} only guarantees that there are no API errors in the
...
with a {\sf GrB\_OUTOFMEM} or a {\sf GrB\_INDEX\_OUTOFBOUNDS} error,
...
{\sf GrB\_INVALID\_VALUE} & Miscellaneous incorrect values. \jose {Maybe this could be used to catch invalid indices as API errors.} \\
{\sf GrB\_INDEX\_OUT\_OF\_BOUNDS} & Indices passed are larger than dimensions of matrix or vector being accessed. \jose{Remove from API errors.} \\
...
\begin{tabular}{l|p{3in}}
Error code	& Description \\ \hline
{\sf GrB\_OUTOFMEM}		& Not enough memory for operations. \\
{\sf GrB\_INDEX\_OUTOFBOUNDS}	& Reference to a vector or matrix elements that is outside the defined dimensions of the object. \\
