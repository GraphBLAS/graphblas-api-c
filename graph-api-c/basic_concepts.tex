...
\begin{threeparttable}
\hrule
\caption{Predefined monoids for GraphBLAS in C. Maximum and minimum values for the various integral types are defined in {\tt stdint.h}. Floating-point infinities are defined in {\tt math.h}. \scott{We need to decide what takes the place of +/- infinity identities for domains that aren't supported in C. The only place we need to expose this implementation detail is in reduce to scalar, I think.  We need to decide about two boolean monoids for NE and EQ.  We can do LXOR for one. I have added LXNOR to the table of binary operators for the other.}}
