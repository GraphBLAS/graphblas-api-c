...
The GraphBLAS C API is designed to work with applications that run
as multiple threads executing within a shared address space.
...
across threads.  The GraphBLAS specification does not define its own memory consistency model. Instead it specifies
what must be done by a programmer calling GraphBLAS methods and by the implementor of 
a GraphBLAS library so an implementation of the GraphBLAS
can work correctly with the memory consistency model of the host environment. 

A memory consistency model is defined in terms of happens-before relations between methods in different
...
as \emph{sequential consistency} can be used in place of the acquire-release semantics.
...
the non-opaque value is needed but the object from which it is computed is not needed. We
want implementations of the GraphBLAS to be able to exploit such optimizations when they 
are allowed.
