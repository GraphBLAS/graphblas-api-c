...
Sequences that do not conflict are free of data races.
A data race occurs when two or more sequences access to the same GraphBLAS object
and at least one of the sequences modify that object. Programs that are race-free do not 
run multiple sequences that modify the same GraphBLAS object
where at least one of the sequences modify that object. 
The GraphBLAS C API does not define synchronization 
constructs to order operations between concurrent sequences.  
