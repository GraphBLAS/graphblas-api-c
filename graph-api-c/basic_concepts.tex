...
they are encountered during execution of a program, as specified by the
text of the program.  This is called the \emph{program order}.  Each
method in the collection uniquely and unambiguously defines the output
GraphBLAS objects based on the GraphBLAS operation and the input GraphBLAS
objects. This is the case as long as there are no execution errors, which
can put objects in an invalid state (see Section~\ref{Sec:ErrorModel}).
...
GraphBLAS objects are fully defined at any point in the execution of a
thread by the semantics of the methods that were invoked prior to that
point,  as long as there are no execution errors.  In particular, as soon
as a GraphBLAS method call returns, its output can be used in the next
GraphBLAS method call.  However, in \emph{non-blocking mode}, individual
method calls may not be \emph{complete} even after they return. We say
that a method call with output GraphBLAS object {\sf obj} in non-blocking
mode is complete when the first subsequent call to {\sf GrB\_wait(obj)}
has returned.  Once a method call is complete, its output object must be
materialized. Furthermore, no additional execution time can be charged
to the method call and no additional errors can be attributed to the
method call.
...
method call.  A call to {\sf GrB\_wait(obj)} guarantees completion of
all previous, in program order, method calls that have GraphBLAS object
{\sf obj} as an output ({\sf OUT} or {\sf INOUT}) argument.
...
{\sf obj} is followed by a {\sf GrB\_wait(obj)} call is equivalent to the
same sequence in blocking mode with {\sf GrB\_wait(obj)} calls removed.
...
returned value can be either {\sf GrB\_SUCCESS} 
(a value of 0)\scott{NEW CONTENT} or one of the defined
values shown in Table~\ref{Tab:ErrorValues}. The return values fall into
two groups: API errors (Table~\ref{Tab:ErrorValues}(a)) and execution
errors (Table~\ref{Tab:ErrorValues}(b)). 
\scott{NEW CONTENT: we need to discuss negative vs. positive values.  Positive values are also informational.  Should we even bother with the distinction?}
...
\vspace{1\baselineskip}
(b) Execution errors/return values
\vspace{1\baselineskip}
...
(or any other program data) have been modified.
