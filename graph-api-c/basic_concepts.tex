...
%TGM: I updated the sequence definition to match the definition in the text.
\glossItem{sequence} A \emph{sequence} is a series of GraphBLAS method calls in program order.  An implementation of the GraphBLAS
may reorder or even fuse GraphBLAS methods within a sequence as long as the definitions of any GraphBLAS object that are later read by 
an application are not changes; where by ``read'' we mean that values are copied from an opaque GraphBLAS object into a non-opaque object.
A sequence begins when a thread calls the first method that creates
or modifies a GraphBLAS object either (1) the first call in an application or (2) the first call following prior sequence termination.  The only way
to terminate a sequence within an application is with a call to the GrB\_wait() method. 
...
%TGM: A new definition of complete.  When we refere to extract as "sequence terminating" we need to remove that 
%   language and instead say that the method forces the object to be complete.
\glossItem{complete} A GraphBLAS object is fully defined by the sequence of methods within which it appears.   The execution of a sequence
may be deferred, however, so at any point in an application the values associated with a particular GraphBLAS object
may not have been computed and stored in memory.   We say that a GraphBLAS object is \emph{complete} when the 
computations that implement the mathematical definition of the object have finished and the values associated with 
that object are available to any method that would load them into a non-opaque data structure.   Essentially, methods
that extract elements from an opaque object and copy them into a non-opaque object force \emph{completion} of the
opaque object. 
...
%
% TGM: I added this to reduce confusion about the fact our semirings are not always
%     strictly mathematically correct semirings
...
elements are useful in graph algorithms even when they do not strictly define a semiring.
...
\section{Notation}
%
% TGM:  I filled in this table as best I could.  I really need someone to check this to make sure it's right.  Also, if we
%     don't use indrow and indcol (defined in section 3.5 but possibly not used) we should remove them here and in
%     section 3.5.
...
Similar to GraphBLAS types having predefined types and user-defined types, GraphBLAS operators come in two types as well: (1) predefined operators found in Table~\ref{Tab:PredefinedOperators} and (2) user-defined operators using {\sf GrB\_UnaryOp\_new()} or {\sf GrB\_BinaryOp\_new()} (see Section~\ref{Sec:AlgebraMethods}).
...
%For {\sf extract} and {\sf assign} GraphBLAS operations, the constant {\sf GrB\_ALL} is a constant for a null index pointer:
%
%\begin{verbatim}
%    GrB_Index *GrB_ALL = (GrB_Index*)0;
%\end{verbatim}

\scott{Other non-opaque types that need to be defined:}
...
\scott{Other constants that need to be defined:}
...
%
%  TGM: Here is the original text with a somewhat vague definition of sequence.  The updated
%     text follows this commented block of text
%
...
% 
% Jose: I commented out the previous version and I am writing down a version that 
%       is based more on specific call sequences.
%
...
output object have been produced and committed to the address space of the program.
...
\comment{
For any GraphBLAS object, we call the ordered collection of method calls that mathematically 
define the value of the object, the \emph{sequence} for that object. We say that the 
sequence  \emph{fully defines} a GraphBLAS object.  We further define that for
any give GraphBLAS operation, that operation is \emph{complete} when the computations 
associated the operation have finished and the values of any associated GraphBLAS objects are available 
for use by subsequent GraphBLAS methods.

The sequence for an object is initiated when that object is created or by the first method in the sequence that
modifies the object.  The sequence for an object terminates when: (1) the first GraphBLAS method 
returns that reads values from the GraphBLAS object and writes them into a non-opaque data 
structure or (2) a GraphBLAS wait method (e.g. {\sf GrB\_wait()}) that follows the sequence in program order
returns.  We refer to GraphBLAS methods that write values from opaque objects into nonopaque objects
as \emph{terminating methods} since they may terminate the sequence of any opaque object
from which they read values.

GraphBLAS objects frequently depend on other GraphBLAS objects.  In these cases, the 
sequences for the individual objects may depend on each other such that termination of one
sequence may require completion of operations in another sequence.   As suggested above, a
call to the GraphBLAS wait method terminates all sequences defined in program order prior to the
call to the GrahBLAS wait method.
}

\subsection{Execution modes}

% A program using the GraphBLAS C API defines a collection of sequences.  
The execution model
implied by GraphBLAS sequences depends on the \emph{execution mode} of the GraphBLAS program.  There are
...
call.  Completion of all operations in a sequence, including any execution
...
deferred, even in nonblocking mode. That is, if a method is called with
an API error, it always returns with an API error value.  If a GraphBLAS
method returns with an API error, it is guaranteed that none of the
method arguments (or any other program data) have been modified.
...
memory leak is a program error but it may manifest itself in different
points of program execution (or not at all) depending on the platform,
...
information concerning the cause of the error.  Implementation-specific
additional information is returned in the null-terminated character
string {\sf err} in the sequence-terminating {\sf GrB\_wait()} method
call.  If a GraphBLAS method, executing in nonblocking mode, returns
