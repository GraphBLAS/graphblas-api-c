...
\begin{itemize}
\item Glossary of terms used in the standard.  
...
to expose GraphBLAS objects to external packages.  
...
\glossItem{context} The state of an instance of the GraphBLAS C API in a 
program.  This is set with GrB\_init() and at this time consists of the mode of the 
program execution.   \tim{I'm not done with this yet ... this is a place holder definition}
...
\glossItem{GraphBLAS operators} The fundamental operators that act on elements
of GraphBLAS objects.  These are usually associated with algebraic structures used in the 
GraphBLAS such as monoids and semirings.   GraphBLAS operators may be 
predefined or they may be user-defined binary or unary functions.
...
on matrices and vectors with elements defined in terms of an algebraic semi-ring.

\glossItem{method} A C function defined in the GraphBLAS C API that manipulates
an opaque GraphBLAS object.

\glossItem{mode} blocking and nonblocking

\glossItem{monoid} An algebraic structure consisting of a domain, an associative
binary operation, and an identity corresponding to that operation. 
...
over addition (\emph{)}) and multiplication (\emph{1}), and finally the additive
identity is an annihilator over multiplication.   

\glossItem{sequence}: A GraphBLAS sequence is a sequence of GraphBLAS
method calls in program order.  A sequence begins with the first GraphBLAS method
that modifies a graphBLAS object and terminates with a GraphBLAS method that
explicitly waits for the sequence of methods to complete or any method that 
reads values from a GraphBLAS object into a non-opaque object.
...
the implied \emph{zero} in a sparse representation of a matrix.
...
output from a (possibly different) second domain.  The predefined operators 
of GraphBLAS are listed in Table~\ref{Tab:PredefinedOperators}.
...
In addition to the predefined operators, the GraphBLAS C API supports creation
of user defined binary and unary functions that can be used anywhere a GraphBLAS operator is allowed.
...
\section{Execution and Error Models}
...
\tim{ Note to the team:  I have not written the GraphBLAS error model paragraph yet.  This is just the note from 
our email discussion.  I will get to this later, but I wanted to pause in my writing and make sure I haven't 
messed anything up with my basic definitions}

The return value from a GraphBLAS method communicates information available to the system at 
the time the method returns.  In blocking mode, that information pertains to the full computation and 
the error parameters defined with the method provide information concerning the condition of the computation.  
In nonblocking mode, the information pertains to a consistency check of the arguments to the method.Any method that terminates a sequence must return information about the status of that sequence.  
GrB\_sucess means the method returned correctly and the sequence produced the result defined by the 
sequence of graphBLAS operations.   Other return values from the method indicate that an error 
...
?err? which is always the last argument to any method that may terminate a sequence.  
