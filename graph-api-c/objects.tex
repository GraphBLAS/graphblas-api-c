...
{\sf GrB\_ALREADY\_SET} error code.
...
\label{Sec:Strings}
When the input to {\sf GrB\_<OBJ>\_set} is a {\sf char*} the input array is null terminated. 
...
a properly sized buffer. The returned string is null terminated, so the buffer must be at least
...
\subsection{{\sf Input Types}}
\label{Sec:getset_types}
By default arguments are passed in a {\sf GrB\_Scalar}, only {\sf char*} and GraphBLAS objects 
like {\sf GrB\_Type} are passed in directly. Enums are passed as {\sf GrB\_INT32} {\sf GrB\_Scalar}s.
...
{\sf GrB\_NAMESIZE}                    & --- \(\mid\) ---   & 10    & {\sf All}, {\sf Global}               & {\sf GrB\_INT32}        \\ 
{\sf GrB\_NAME}                        & {\sf *}            & 11    & {\sf All}                             & Null terminated char* of size {\sf GrB\_NAMESIZE} \\
{\sf GrB\_LIBRARY\_NAME}               & --- \(\mid\) ---   & 100   & {\sf Global}                          & Null terminated char* of size {\sf GrB\_NAMESIZE} \\
{\sf GrB\_LIBRARY\_VER\_MAJOR}         & --- \(\mid\) ---   & 101   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_LIBRARY\_VER\_MINOR}         & --- \(\mid\) ---   & 101   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_LIBRARY\_VER\_PATCH}         & --- \(\mid\) ---   & 101   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_API\_VER\_MAJOR}             & --- \(\mid\) ---   & 102   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_API\_VER\_MINOR}             & --- \(\mid\) ---   & 102   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_API\_VER\_PATCH}             & --- \(\mid\) ---   & 102   & {\sf Global}                          & {\sf GrB\_INT32} \\
{\sf GrB\_BLOCKING\_MODE}              & --- \(\mid\) ---   & 103   & {\sf Global}                          & {\sf GrB\_Mode} \\
{\sf GrB\_NTHREADS}                    & W \(\mid\) H       & 104   & {\sf Global}, {\sf GrB\_Descriptor}   & {\sf GrB\_INT32} \\ \hline
...
\hline
{\sf GrB\_NAMESIZE}                    & The length of the {\sf GrB\_NAME} for a particular object. \\
...
{\sf GrB\_NTHREADS}                    & The number of threads for the library to use for a \\
                                       & function call, may be ignored by the library. \\
...
{\sf GrB\_STORAGE\_FORMAT\_HINT}       & Hint to the library that it should use a specific storage format. \\
{\sf GrB\_ELTYPE}                      & The element type of a collection. \\
{\sf GrB\_INPUT1TYPE}                  & The type of the first argument to an operator. \\
{\sf GrB\_INPUT2TYPE}                  & The type of the second argument to an operator. \\
{\sf GrB\_OUTPUTTYPE}                  & The type of the output of an operator. \\
{\sf GrB\_BINARYOP}                    & The binary operator of a semiring or monoid. \\
{\sf GrB\_MONOID}                      & The monoid of a semiring. \\
\hline
\end{tabular}
\end{center}
