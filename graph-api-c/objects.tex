...
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_ROWINDEX\_Z}*    & $I_{64}^2 \times I_{32/64} \rightarrow I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(i + s)$, & replace with its row index (+ s) \\
                          &                            & $I_{64}   \times I_{32/64} \rightarrow I_{32/64}$ & $f(u_{i}, i,0,s)$ & $=$ & $(i + s)$ \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_COLINDEX\_Z}**   & $I_{64}^2 \times I_{32/64} \rightarrow I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(j + s)$, & replace with its column index (+ s) \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_DIAGINDEX\_Z}**  & $I_{64}^2 \times I_{32/64} \rightarrow I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(j - i + s)$, & replace with its diagonal index (+ s) \\
\hline
...
{\footnotesize
\hspace{0.065in}* Operators that can be used with both {\sf GrB\_Vector} and {\sf GrB\_Matrix}.\newline 
** Operators requiring access to two indices only work with {\sf GrB\_Matrix} and will result in undefined behavior if used with {\sf GrB\_Vector}. \scott{Is this still true if j=0 anytime vector is used?}
}
\end{threeparttable}
\end{table*}
...
\hrule
\vspace{1\baselineskip}
\caption{SECOND ALTERNATIVE TO PREVIOUS TABLE: Documenting which parameters are which types ($-$ is don't care). When operating on vectors, $j=0$.}
\vspace{1\baselineskip}

\begin{tabular}{l|l|cccc|rcll}
Operator type             & GraphBLAS name           & \multicolumn{4}{c|}{Domains}                           & \multicolumn{4}{c}{Description} \\ 
                          &                          & $A,u$ & $i$, $j$    & $s$         & result      & &&& \\ \hline
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_ROWINDEX\_Z}   & $-$   & $I_{U64}^2$ & $I_{32/64}$ & $I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(i + s)$, & replace with its row index (+ s) \\
                          &                          & $-$   & $I_{U64}  $ & $I_{32/64}$ & $I_{32/64}$ & $f(u_{i}, i,0,s)$ & $=$ & $(i + s)$  & \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_COLINDEX\_Z}   & $-$   & $I_{U64}^2$ & $I_{32/64}$ & $I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(j + s)$, & replace with its column index (+ s) \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_DIAGINDEX\_Z}  & $-$   & $I_{U64}^2$ & $I_{32/64}$ & $I_{32/64}$ & $f(A_{ij},i,j,s)$ & $=$ & $(j - i + s)$, & replace with its diagonal index (+ s) \\
\hline

{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_TRIL}    & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j \leq i + s)$, & triangle on or below diagonal s \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_TRIU}    & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j \geq i + s)$, & triangle on or above diagonal s \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_DIAG}    & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j  ==  i + s)$, & diagonal s \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_OFFDIAG} & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j \neq i + s)$, & all but diagonal s \\

{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_COLLE}   & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j \leq s)$, & columns less or equal to s \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_COLGT}   & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(j >    s)$, & columns greater than s \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_ROWLE}   & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(i \leq s)$, & rows less or equal to s \\
                          &                    & $-$ & $I_{U64}$   & $I_{64}$ & {\sf bool} & $f(u_{i}, i,0,s)$ & $=$ & $(i \leq s)$  \\
{\sf GrB\_IndexUnaryOp}   & {\sf GrB\_ROWGT}   & $-$ & $I_{U64}^2$ & $I_{64}$ & {\sf bool} & $f(A_{ij},i,j,s)$ & $=$ & $(i >    s)$, & rows greater than s \\
...
\scott{How can I export a sparse GraphBLAS matrix to a dense format without
specifying what the unstored value is?}
...
\hrule
\begin{center}
\caption{Descriptors are GraphBLAS objects passed as arguments to GraphBLAS 
...
with descriptors. \scott{The values are what SuiteSparse currently uses.}}
