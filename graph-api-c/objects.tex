...
GraphBLAS objects and implementations contain internal fields which may provide
information to users and allow setting runtime parameters and hints.
All GraphBLAS objects are required to implement the {\sf get} and {\sf set}
methods required to query and set these fields. 

A GraphBLAS object may contain a number of (\emph{field}, \emph{value}) pairs,
where the \emph{value} type is determined by the \emph{field}. Objects must
implement a set of such pairs as determined by the specification, but may extend
that set with implementation specific pairs.

The GraphBLAS implementation itself contains several (\emph{field}, \emph{value}) pairs,
...
A field must always be readable, but in many cases may not be writable.
...
{\sf GrB\_NAMESIZE} will be 0. Collections and {\sf GrB\_Descriptor}s 
may have their {\sf GrB\_NAME} set at any time. Algebraic objects and {\sf GrB\_Type}s
may only have their {\sf GrB\_NAME} set once to a globally unique value. 
Attempting to set this field after it has already been set will return a
{\sf GrB\_OUTPUT\_NOT\_EMPTY} error code.

\subsection{String Handling}
\label{Sec:Strings}
When the input to {\sf GrB\_<OBJ>\_set} is a {\sf char*} the array must be null terminated. 
The {\sf GraphBLAS} implementation must copy this array into internal data structures.

When a {\sf char*} is the output argument of {\sf GrB\_<OBJ>\_get} the user must preallocate
a properly sized buffer. The returned string must be null terminated, so the buffer must be at least
1 larger than the size of the string. For instance the buffer for {\sf GrB\_NAME} must be of length
{\sf GrB\_NAMESIZE + 1}.

\subsection{Hints}
\label{Sec:Hints}
Several fields are {\em hints} (marked {\sf H} in~\ref{Tab:Fields}). 
A GraphBLAS implementation is free to ignore a hint and
return {\sf GrB\_SUCCESS}. For instance {\sf GrB\_NTHREADS} might be ignored by a sequential
GraphBLAS implementation.

\subsection{{\sf get} after {\sf set}}
\label{Sec:GetSet}
If a field allows writes, or {\sf set}, {\sf get} must return the most recent value written.
If a field is a {\em hint}, the hint is returned, not the value used by the library internally.

An exception to this is fields which return the {\sf GrB\_ALREADY\_SET} return code if they are set
more than once. In Tables~\ref{Tab:Fields}, {\sf GrB\_NAME} for {\sf Algebraic} types and {\sf GrB\_Type}
may only be set once, and will return {\sf GrB\_ALREADY\_SET} on subsequent calls to {\sf GrB\_set}.
...
which may be ignored by the implementation. For {\sf *} see~\ref{Sec:GetSet}}
\label{Tab:Fields}
\vspace{1\baselineskip}
...
{\sf GrB\_OUTP}                        & W \(\mid\) ---     & 0     & {\sf GrB\_Descriptor}                 & {\sf GrB\_Desc\_Value} \\
{\sf GrB\_MASK}                        & W \(\mid\) ---     & 1     & {\sf GrB\_Descriptor}                 & {\sf GrB\_Desc\_Value} \\
{\sf GrB\_INP0}                        & W \(\mid\) ---     & 2     & {\sf GrB\_Descriptor}                 & {\sf GrB\_Desc\_Value} \\
{\sf GrB\_INP1}                        & W \(\mid\) ---     & 3     & {\sf GrB\_Descriptor}                 & {\sf GrB\_Desc\_Value} \\ \hline
{\sf GrB\_NAMESIZE}                    & --- \(\mid\) ---   & 10    & {\sf All}                             & {\sf GrB\_Index}        \\ 
{\sf GrB\_NAME}                        & {\sf *}            & 11    & {\sf All}                             & Null terminated char* of size {\sf GrB\_NAMESIZE} \\ 
                                       &                    &       &                                       & Minimum supported size of 512-bytes \\\hline
{\sf GrB\_LIBRARY\_NAME}               & --- \(\mid\) ---   & 100   & {\sf Global}                          & 256-byte null terminated char* \\
{\sf GrB\_LIBRARY\_VER}                & --- \(\mid\) ---   & 101   & {\sf Global}                          & Length 3 integer array \\
{\sf GrB\_API\_VER}                    & --- \(\mid\) ---   & 102   & {\sf Global}                          & Length 3 integer array \\
{\sf GrB\_BLOCKING\_MODE}              & --- \(\mid\) ---   & 103   & {\sf Global}                          & {\sf GrB\_Mode} \\
{\sf GrB\_NTHREADS}                    & W \(\mid\) H       & 104   & {\sf Global}, {\sf GrB\_Descriptor}   & {\sf GrB\_Index} \\ \hline
{\sf GrB\_STORAGE\_ORIENTATION\_HINT}  & W \(\mid\) H       & 200   & {\sf Global}, {\sf Collection}        & {\sf GrB\_ROWMAJOR}, {\sf GrB\_COLMAJOR} \\
{\sf GrB\_STORAGE\_FORMAT\_HINT}       & W \(\mid\) H       & 201   & {\sf Collection}                      & {\sf GrB\_Format} \\
{\sf GrB\_ELTYPE??}                    & --- \(\mid\) ---   & 202   & {\sf Collection}                      & {\sf GrB\_Type} \\ \hline
{\sf GrB\_INPUT1TYPE??}                & --- \(\mid\) ---   & 300   & {\sf Algebraic}                       & {\sf GrB\_Type} \\
{\sf GrB\_INPUT2TYPE??}                & --- \(\mid\) ---   & 301   & {\sf Algebraic}                       & {\sf GrB\_Type} \\
{\sf GrB\_OUTPUTTYPE??}                & --- \(\mid\) ---   & 302   & {\sf Algebraic}                       & {\sf GrB\_Type} \\
{\sf GrB\_BINARYOP??}                  & --- \(\mid\) ---   & 303   & {\sf GrB\_Monoid}, {\sf GrB\_Semiring}& {\sf GrB\_BinaryOp} \\
{\sf GrB\_MONOID??}                    & --- \(\mid\) ---   & 304   & {\sf GrB\_Semiring}                   & {\sf GrB\_Monoid} \\
\end{tabular}
\end{center}
\hrule
\end{table}
