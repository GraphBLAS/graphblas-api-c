...
\scott{This section needs work.}

A mask can either be a one- or two-dimensional construct, like vector
matrix, except that is only has structure and no domain or values.  

A one-dimensional mask $\vector{m} = \langle N, \{ i \} \rangle$
is defined as its number of elements $N>0$ and a set of indices $(i)$ 
where $0 \leq i < N$.  A particular value of $i$ can only
appear at most once in $\vector{m}$. We define $\bold{n}(\vector{m}) = N$. 
We also define the (structure) set
$\vector{i(\vector{v})} = \bold{s}(\vector{v}) = \{ i : (i) \in \bold{L}(\vector{v}) \}$.

A two-dimensional mask $\matrix{M} = \langle M, N, \{ (i,j) \} \rangle$,
is defined by its number of rows $M>0$, its number of columns
$N>0$ and a set of tuples $(i,j)$ where $0 \leq i < M$, $0 \leq
j < N$.   A particular pair of values $i,j$ can only
appear at most once in $\matrix{M}$.  We define $\bold{n}(\matrix{M})
= N$, $\bold{m}(\matrix{M}) = M$, and (structure) 
$\bold{S}(\matrix{M}) = \{(i,j)\}$.  
We also define the sets $\vector{i(\matrix{M})} = \{
i : \exists (i,j) \in \matrix{M} \}$ and $\vector{j(\matrix{M})}
= \{ j : \exists (i,j) \in \matrix{M} \}$.  These are the sets
of nonempty rows and columns of $\matrix{M}$, respectively.

One common operation on masks is the \emph{strutural complement}.  For two dimensional
masks this is denoted $\neg\matrix{M}$. The structure of a complemented mask is
defined as $\bold{S}(\neg\matrix{M}) = \{(i,j)$ where $0 \leq i < M$, $0 \leq
j < N$, and $(i,j) \notin \bold{S}(\matrix{M}) \}$.  It is the set of all possible
...
\scott{We need to double check that the above matrix still applies after
final reordering of parameters.}
