...
\scott{It may be that this specification documents that the largest maximum is (1 << 63)-1 i.e. 0xefff ffff ffff ffff. This is related to possible restrictions on how index unary operators are performed. Also should there be a compile-time and run-time constant like the version numbers?}
%\scott{I need better wording. Should we specifically require a \#define or be looser? Tim Davis defines this as (1ULL << 60) but I would have thought that it should be ((1ULL << 60) - 1).  It looks like Tim D. is defining one past end which is conceptually {\sf GrB\_MAX\_NUM\_VERTICES}}
