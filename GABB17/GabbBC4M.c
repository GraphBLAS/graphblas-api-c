...
GrB_info BC_update(GrB_Vector *delta, GrB_Matrix A, GrB_Index *s, GrB_Index nsver) |\label{line:sig}|
{
  GrB_Index n; 
  ...
  do {  // --------------------- The BFS phase (forward sweep) --------------------------- |\label{line:dowhile}|
    ...
    GrB_apply(&(sigmas[d]),GrB_NULL,GrB_NULL,GrB_IDENTITY_BOOL,frontier,GrB_NULL); // sigma[d](:,:) = (Boolean) frontier  |\label{line:sigma_set}|
