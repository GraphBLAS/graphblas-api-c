...
and descriptors, enables codification of the algorithm with fewer function calls within the main loops. The 
...
breadth-first search traversals (one for each source vertex) where it also keeps track 
...
An alternate approach is shown here using the GraphBLAS {\tt extract}
...
The first step is to initialize the {\tt n}$\times${\tt nsver} {\tt sigmas[d]} matrix 
...
used to ensure only paths that identified in the BFS phase are assigned to
the result.
...
(defined in line~\ref{line:desc}) that uses the actual 
adjacency matrix (as opposed to its transpose). The algorithm makes sure that the BC contributions are 
...
to perform this computation.  Note that {\tt GrB\_free\_all} is a convenience macro (not part of GraphBLAS C API) that 
