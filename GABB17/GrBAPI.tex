...
\begin{itemize}[leftmargin=1.1in]
    ...
    into existing \matrix{\sf C} entries. For assignment rather than accumulation, 
    {\sf GrB\_NULL} is specified.
...
\begin{itemize}[leftmargin=2.1in]
	...
	\item[{\sf GrB\_SUCCESS}]	blocking mode: the operation
	completed successfully. Nonblocking mode: consistency tests
	passed on dimensions and domains for the input arguments.  
...
all have descriptor-driven options to modify input matrices or vectors, and write masks.   
Low level details may vary, but the basic pattern used in {\sf GrB\_mxm} holds.
 
\begin{table}[h]
\hrule
\begin{center}
\caption{The following methods are used in the betweenness centrality example in section~\ref{sec:example}.}
...
\hline
	...
	{\sf GrB\_mxm}              & Multiplies a matrix with another matrix on a semiring.                              \\
	...
	{\sf GrB\_apply}            & Apply a unary operator to the elements of a matrix.                                 \\
	{\sf GrB\_reduce}           & Reduce elements across each row of a matrix to produce a vector.                    \\
\end{tabular}
\end{center}
\hrule
\end{table}
