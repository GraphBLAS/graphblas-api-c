...
are opaque objects accessible only through the methods in the GraphBLAS C API.
...
\begin{itemize}
\item The domain $D$, the data type for the vector elements.
\item The size $N>0$.
\item A set of tuples $(i,v_i)$ where $0 \leq
i < N$ and $v_i \in D$. 
\end{itemize}
A particular value of $i$ can appear only 
...
defined by
\begin{itemize}
\item The domain $D$, the data type for the matrix elements.
...
$$
It has two domains, $D_1$ and $D_2$, and an operation
...
$$
the set of all possible indices that do not appear in $\matrix{M}$.
...
In particular, descriptors specify how other input 
arguments---vectors, matrices, and masks---should be processed (modified) 
...
input matrix needs to be transposed or that the structural complement of a mask should be used.
...
{\sf GrB\_OUTP}; the mask by the field name {\sf GrB\_MASK}; and the input 
...
first GraphBLAS method call and
...
define the output GraphBLAS objects.  
A GraphBLAS program executes in one of 
two modes: \emph{blocking} or \emph{nonblocking}.  
\begin{itemize}
...
objects are completely computed and stored in memory before each method returns.

\item \emph{nonblocking}: Methods \emph{may} return after input arguments 
have been verified.   Methods that manipulate only opaque objects \emph{may} defer their 
...
output GraphBLAS objects  are not completely computed until (1) the sequence is terminated or (2) a GraphBLAS method 
that copies values from a GraphBLAS object into a non-opaque object returns.   At that point, the GraphBLAS object 
is said to be \emph{complete} (i.e., values associated with that object are completely computed and stored in memory).  
\end{itemize}

In a  mathematically well-defined sequence with input objects that are numerically well-conditioned, the results
from blocking and nonblocking modes should be identical except for effects due to round-off errors 
...
This mode is valuable for debugging or when an external tool needs to 
...
with a call to {\sf GrB\_finalize()}.  The context can be 
set only once in the execution of a program (i.e., after {\sf GrB\_finalize()} is called, a subsequent call to
...
is called.  If no API errors are found, the method may return at this point with the value 
of {\sf GrB\_SUCCESS}.  Since execution in nonblocking mode may be deferred, the only 
information guaranteed to be available when a method returns pertains to the API test, and the return value may not provide 
...
a call to {\sf GrB\_error()}, which returns a pointer to a null terminated 
...
API errors.  When these methods---such as methods to extract values from an 
opaque object into a non-opaque object---force completion of a GraphBLAS object, 
the return value is {\sf GrB\_SUCCESS} if no execution errors were generated by any 
