...
In this section, we introduce the basic objects.  Then we consider the GraphBLAS C API
execution and error models.
...
\begin{itemize}
\item  a domain $D$, 
\item a number of rows $M>0$ and columns $N>0$
...
$$
its \emph{transpose} is another matrix 
$$
...
A GraphBLAS \emph{binary operators} 
$$
...
A GraphBLAS \emph{unary operators} 
$$
...
A GraphBLAS \emph{generalized monoid} (or \emph{monoid} for short)
$$
...
and an identity element $0 \in D_1$.  For a given GraphBLAS monoid
$$
M=\langle D_1,\odot,0 \rangle
$$ 
...
$$ 
be a GraphBLAS binary operator
...
control optional behaviors of operations. 
...
There are two modes A GraphBLAS program can use for execution: \emph{blocking} and \emph{nonblocking}.  
\begin{itemize}
