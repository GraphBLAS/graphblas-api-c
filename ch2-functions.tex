...
\section{Matrix Operations Overview}
...
$$
where  ${\bf A}: \mathbb{R}^{N \times M}$,  ${\bf B}: \mathbb{R}^{M \times L}$, and ${\bf C}: \mathbb{R}^{N \times L}$. SpGEMM has many important variants which including accumulating, transposing inputs or outputs, and user defined addition and multiplication.  These variants can be used alone or in combination.

\subsection{Accumulation}

SpGEMM can be used to multiply and accumulate values into a matrix.  One example is when the result of multiply ${\bf A}$ and ${\bf B}$ is added to ${\bf C}$ (instead of replacing ${\bf C}$.  This can be written
$$
   {\bf C} ~~ {+}{=} ~~ {\bf A} {\bf B}
$$
or more explicitly
...
\subsection{Transposing inputs or outputs}
...
\subsection{Alternate Addition and Multiplication}
