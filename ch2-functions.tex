...
\end{description}
The above set of functions has been shown to be useful for implementing a wide range of graph algorithms.  These functions strike a balance between providing enough functions to be useful to an application builders and while being few enough that they can be implemented effectively.  Furthermore, from an implementation perspective, there are only four functions that are truly fundamental: {\bf Sparse}, {\bf Find}, {\bf SpGEMM}, and {\bf SpEWwiseX}.  The other GraphBLAS functions can be implemented from these functions four functions.
...
$$
where $\mathbf{P}({\bf i})$ and $\mathbf{P}({\bf j})$ are permutation matrices given by
$$
