...
\begin{description}
\item[Sparse]  Constructs a {\bf Sparse} matrix from triples
\item[Find] {\bf Find} returns all non-zero entries in a sparse matrix as triples
...
\section{Sparse}
  The GraphBLAS may use a variety of internal formats for representing sparse matrices.  This data can often be imported as triples of vectors ${\bf i}$, ${\bf i}$, and ${\bf v}$ corresponding to the non-zero elements in the sparse matrix.  Constructing an NxM sparse matrix from triples can be denoted
$$
...
$$
where $\mathbb{S} \rightarrow [0,\infty)$; and power set algebras
$$
