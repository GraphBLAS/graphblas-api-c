...
\begin{figure}[htb]
  \centering
    \includegraphics[width=3in]{figures/AdjacencyMatrix.pdf}
      \caption{(left) Seven vertex graph with 12 edges.  Each vertex is labeled with with an integer.  (right)  $7 \times 7$ adjacency matrix ${\rm A}$ representation of the graph. ${\rm A}$ has 12 non-zero entries corresponding to the edges in the graph.}
...
\begin{figure}[!htb]
  \centering
    \includegraphics[width=4.5in]{figures/IncidenceMatrix.pdf}
      \caption{(left) Seven vertex graph with 12 edges.  Each edge is labeled with with an integer; the vertex labels are the same as in Figure~\ref{fig:AdjacencyMatrix}.  (middle)  $12 \times 7$ incidence matrix $\mathbf{E}_{\rm start}$ representing the starting vertices of the graph edges.   (right)  $12 \times 7$ incidence matrix $\mathbf{E}_{\rm end}$ representing of the ending vertices of the graph edges. Both $\mathbf{E}_{s\rm tart}$ and $\mathbf{E}_{\rm end}$ have 12 non-zero entries corresponding to the edges in the graph.}
      \label{fig:IncidenceMatrix}
\end{figure}
\begin{figure}[!htb]
  \centering
    \includegraphics[width=4.5in]{figures/IncidenceMatrixMultiHyper.pdf}
      \caption{Graph and incidence matrices from Figure~\ref{fig:IncidenceMatrix} with a hyper-edge (edge 12) and a mulit-edge (edge 13).  The graph is a hyper-graph because edge 12 has one more than one end vertex.  The graph is a multi-graph because edge 8 and edge 13 are identical.}
...
\section{Assign: Modifying Sub-Graphs}
  Modifying sub-graphs is a very common graph operation.  The GraphBLAS performs this operation with the {\bf Assign} function by selecting starting vertices (row) and ending vertices (columns) from a matrix $\mathbf{C} : \mathbb{S}^{N \times M}$ and assigning new values to them from another sparse matrix, $\mathbf{A} : \mathbb{S}^{N_A \times M_A}$
$$
...
$$
where $i \in \{1,...,N_A\}$, $j \in \{1,...,M_A\}$, ${\bf i} : I^{N_A}$ and ${\bf j}: J^{M_A}$ select specific sets of rows and columns in a specific order and ${\textcolor{blue}{\oplus}}$ optionally allows $\mathbf{A}$ to added to the existing values of $\mathbf{C}$. 
